...
class TunnelAPI {
    constructor(tunnelClientId, tunnelClientKey) {
        Object.assign(this, { tunnelClientId, tunnelClientKey });
    }
}

Object.assign(TunnelAPI.prototype, {
    requestConnect: co.wrap(function *(receiveUrl) {
    ...
    _sendRequest: co.wrap(function *(apiPath, apiParam, withTcKey) {
        let url = config.TunnelServerUrl + apiPath;
    ...
    _packReqData(data, withTcKey) {
        ...
        result = { data, 'tcId': this.tunnelClientId };

        if (withTcKey) {
            result.tcKey = this.tunnelClientKey;
        }
