...
class TunnelService extends ServiceBase {
    constructor(req, res) {
        ...
        let tcId = md5(this.receiveUrl);
        let tcKey = config.TunnelSignatureKey;
        this.tunnelApi = new TunnelAPI(tcId, tcKey);
    }
    ...
    static broadcast(tunnelIds, messageType, messageContent) {
        ...
        return this.tunnelApi.emitMessage(tunnelIds, messageType, messageContent);
    }
    ...
    static emit(tunnelId, messageType, messageContent) {
        ...
        return this.tunnelApi.emitMessage([tunnelId], messageType, messageContent);
    }
    ...
    static closeTunnel(tunnelId) {
        ...
        return this.tunnelApi.emitPacket([tunnelId], 'close');
    }
    ...
    handleGet(handler, options) {
        co.wrap(function *() {
            ...
            try {
                let body = yield this.tunnelApi.requestConnect(this.receiveUrl);
                let data = body.data;
